...
- my-stocks.html — 我的股票分頁（登入制個人資產＋LINE 到價通知，見下方「我的股票分頁」章節）
- valuations.json — 全部追蹤股的便宜/合理/昂貴價（由 `node scripts/export-valuations.mjs` 從 TRACKED 產生，**改完 TRACKED 必須重跑**）
- 即時股價 API：https://taiwan-stock-price.david870114.workers.dev/?id={代號}
- 我的股票後端 API：https://my-stocks-api.david870114.workers.dev（Cloudflare Worker，原始碼在 repo 的 `worker/`）
...
| 欄位 | 內容 | 用途 |
|------|------|------|
| 「近四季EPS」欄 | **該月所屬季度的已公告 TTM EPS**（同一季三個月同值，如 26M04–06 都是 26Q2 的 TTM）；尚未公告的月份沿用前值 | ✅ 估值用（便宜/合理/昂貴價） |
| 「本益比換算價格 10x」欄 ÷ 10 | GoodInfo 在季報之間**內插**出的平滑 EPS，逐月遞變 | ✅ 河流圖月線陣列用 |

兩欄在「最新月份」會相等，可用來交叉驗證。

**判斷最新 TTM 屬於哪一季**：看「近四季EPS」欄的變動起點——值從哪個月開始改變，那個月就是新一季的第一個月（M01/M04/M07/M10 分別對應 Q1/Q2/Q3/Q4）。例：8210 在 26M04 由 33.95 變 38.39 → 38.39 是 **26Q2** 的 TTM。若某股最新值從 26M01 起就沒變過（如 2330 的 74.39），代表 **26Q2 尚未公告**，該值仍是 26Q1 的 TTM，不可標成 Q2。

### ⚠️ 景氣循環股不可機械式套用 PER 法

記憶體、面板、被動元件等強循環股，TTM EPS 會在循環高峰/谷底嚴重失真，直接用「最新 EPS × 原倍數」會產生**假訊號**（valuations.json 會餵給 LINE 到價通知，後果是錯誤推播）。套用前先檢核：

- 算出的合理價 > 現價 × 2.5（EPS 恐為循環高峰，會誤判成「便宜」）
- 算出的合理價 < 現價 ÷ 2.5（EPS 恐為循環谷底）
- 單季 EPS 變動 > ±50%
- EPS ≤ 0

命中任一項就**維持原估值帶（以正常化 EPS 為錨）**，並在 TRACKED 該列註記原因，不要自動改。2026/08 已如此處理：3481 群創、2408 南亞科、2337 旺宏、2344 華邦電、3260 威剛；2026/09 再加 6770 力積電（單季 −77%）、1303 南亞（單季 +87%），並將 8 月誤套 26Q2 的 3037 欣興（退回 84/141/211，EPS 7.03×12/20/30x）、8046 南電（退回 79/119/198，2026E EPS 9.88×8/12/20x）退回原估值。頁面上的 TTM EPS 顯示與即時 PER 仍改用最新實際值，只有估值帶不動。

### 月線陣列只放「已收盤的完整月份」

各分析頁 `const data` 與 index.html `ETF_DATA[].raw` 一律只加**上個月的實際月收盤**，不要塞當月未收盤的價格（會被誤當成月收盤且沒人回頭更正）。2026/07 更新時 1519、1513 就曾誤存月中價（721 / 168.5，實際月收為 705 / 153.5），已於 2026/08 修正。當前價由頁面的即時 API fetch 負責顯示。

**改完月線陣列務必檢查**：`ETF_MONTHS` 的 while 迴圈上界要往後推一個月（否則新月份不會進 X 軸），且新增列前一列的**逗號不能掉**。可用下列指令驗證全站 JS 語法：
```bash
node -e "const fs=require('fs');let bad=0;for(const f of fs.readdirSync('.').filter(f=>f.endsWith('.html'))){const s=fs.readFileSync(f,'utf8');for(const m of s.matchAll(/<script(?![^>]*\bsrc=)[^>]*>([\s\S]*?)<\/script>/g)){try{new Function(m[1])}catch(e){console.log('✗',f,e.message);bad++}}}console.log(bad?bad+' 個錯誤':'✓ 全部正常')"
```
...
4. 同步更新 index.html（個股四處 / ETF 五處，見下方），改完 TRACKED 後執行 `node scripts/export-valuations.mjs` 重新產生 valuations.json
...
2. 補充最新月份數據，重新計算估值
3. 更新分析 HTML + index.html，若有改 TRACKED 估值 → 執行 `node scripts/export-valuations.mjs` 重新產生 valuations.json
...
- **法人目標價（TRACKED tp 與總覽「法人目標價」欄）**：統一採 FactSet 目標價中位數，來源 `https://www.cnyes.com/twstock/foreignrating.aspx?code={代號}`（表格可能舊到新排序，要找最新日期），或鉅亨「Factset 最新調查」新聞；欄內標註調查日期。2026/09/17 已更新 11 檔。
- **3231 緯創估值倍數**：2026/09 起改為 14/18/26x（原 10/18/30x 與分析頁歷史區間不一致）。
...
## 我的股票分頁（my-stocks.html + worker/）

登入制個人資產頁：輸入持股/追蹤標的存雲端、圖表分析、LINE 到價通知。
...
```powershell
cd C:\Users\USER\Desktop\taiwan-stock-analysis\worker
npx wrangler login                                  # 首次：瀏覽器 OAuth
npx wrangler kv namespace create MYSTOCKS           # 首次：把回傳 id 填進 wrangler.toml
npx wrangler secret put LINE_CHANNEL_ACCESS_TOKEN   # 首次：LINE Messaging API channel token
npx wrangler deploy                                 # 每次改 worker/src/index.js 後
```

- LINE webhook URL（填在 LINE Developers Console）：`https://my-stocks-api.david870114.workers.dev/line-webhook`
- 使用者對官方帳號傳訊息 → webhook 自動回覆其 LINE User ID → 貼到 my-stocks.html 通知設定
- KV 資料：`user:{帳號}`（密碼雜湊）、`portfolio:{帳號}`（持股/追蹤/通知設定）、`session:{token}`（30 天）、`alertstate:{帳號}:{代號}`
